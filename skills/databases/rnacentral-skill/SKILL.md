...
description: Use RNAcentral through DSH-Rosalind's typed public-service request tool.
---

# RNAcentral

Use `database_request` with `provider: "rnacentral"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `rnacentral` (RNAcentral) at `https://rnacentral.org/api/v1`. Its default relative route is `rna/`; named `operation` values are `rna`, `xrefs`. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
