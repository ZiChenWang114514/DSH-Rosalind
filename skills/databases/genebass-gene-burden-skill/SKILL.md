...
description: Use Genebass gene burden through DSH-Rosalind's typed public-service request tool.
---

# Genebass gene burden

Use `database_request` with `provider: "genebass-gene-burden"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `genebass-gene-burden` (Genebass) at `https://main.genebass.org/api`. Its default relative route is `gene/`; named `operation` values are the provider's default query. Pagination mode is `none`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
