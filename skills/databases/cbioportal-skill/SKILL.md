...
description: Use cBioPortal through DSH-Rosalind's typed public-service request tool.
---

# cBioPortal

Use `database_request` with `provider: "cbioportal"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `cbioportal` (cBioPortal) at `https://www.cbioportal.org/api`. Its default relative route is `studies`; named `operation` values are `mutations`. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
