...
description: Use GWAS Catalog through DSH-Rosalind's typed public-service request tool.
---

# GWAS Catalog

Use `database_request` with `provider: "gwas-catalog"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `gwas-catalog` (GWAS Catalog) at `https://www.ebi.ac.uk/gwas/rest/api/v2`. Its default relative route is `associations`; named `operation` values are the provider's default query. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
