---
name: uniprot-skill
description: Use UniProt through DSH-Rosalind's typed public-service request tool.
---

# UniProt

Use `database_request` with `provider: "uniprot"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `uniprot` (UniProt) at `https://rest.uniprot.org`. Its default relative route is `uniprotkb/search`; named `operation` values are `entry`, `search`, `uniref`, `uniparc`, `stream`. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
