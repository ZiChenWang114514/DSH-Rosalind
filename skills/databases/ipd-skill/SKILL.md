---
name: ipd-skill
description: Use IPD through DSH-Rosalind's typed public-service request tool.
---

# IPD

Use `database_request` with `provider: "ipd"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `ipd` (IPD) at `https://www.ebi.ac.uk/cgi-bin/ipd/api`. Its default relative route is `allele`; named `operation` values are `cell`, `download`. Pagination mode is `none`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
