...
description: Use CELLxGENE through DSH-Rosalind's typed public-service request tool.
---

# CELLxGENE

Use `database_request` with `provider: "cellxgene"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `cellxgene` (CELLxGENE Discover) at `https://api.cellxgene.cziscience.com/curation/v1`. Its default relative route is `collections`; named `operation` values are `collection`. Pagination mode is `none`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
