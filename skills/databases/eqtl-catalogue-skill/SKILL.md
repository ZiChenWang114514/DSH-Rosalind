...
description: Use eQTL Catalogue through DSH-Rosalind's typed public-service request tool.
---

# eQTL Catalogue

Use `database_request` with `provider: "eqtl-catalogue"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `eqtl-catalogue` (eQTL Catalogue) at `https://www.ebi.ac.uk/eqtl/api/v3`. Its default relative route is `associations`; named `operation` values are `dataset`, `associations`. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
