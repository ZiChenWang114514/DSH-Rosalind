---
name: chebi-skill
description: Use ChEBI through DSH-Rosalind's typed public-service request tool.
---

# ChEBI

Use `database_request` with `provider: "chebi"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `chebi` (ChEBI) at `https://www.ebi.ac.uk`. Its default relative route is `chebi/backend/api/public/es_search/`; named `operation` values are `compound`, `search`. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
