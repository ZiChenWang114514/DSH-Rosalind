---
name: ensembl-skill
description: Use Ensembl through DSH-Rosalind's typed public-service request tool.
---

# Ensembl

Use `database_request` with `provider: "ensembl"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `ensembl` (Ensembl) at `https://rest.ensembl.org`. Its default relative route is `lookup/id/`; named `operation` values are `variation`, `lookup`, `xrefs`, `overlap`. Pagination mode is `none`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
