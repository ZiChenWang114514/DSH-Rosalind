---
name: reactome-skill
description: Use Reactome through DSH-Rosalind's typed public-service request tool.
---

# Reactome

Use `database_request` with `provider: "reactome"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `reactome` (Reactome) at `https://reactome.org/ContentService`. Its default relative route is `search/query`; named `operation` values are `event`, `participants`, `pathways`. Pagination mode is `none`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
