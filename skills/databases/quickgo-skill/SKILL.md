---
name: quickgo-skill
description: Use QuickGO through DSH-Rosalind's typed public-service request tool.
---

# QuickGO

Use `database_request` with `provider: "quickgo"`. Select the source requested by the user. Do not silently replace a requested service with another provider, mirror, archive, or identifier resolver.

## Fixed provider contract

The registered provider is `quickgo` (QuickGO) at `https://www.ebi.ac.uk/QuickGO/services`. Its default relative route is `ontology/go/search`; named `operation` values are `term`, `annotation`. Pagination mode is `page-size`. Use only a named operation or a relative path documented by that official service. The runtime rejects undeclared origins and non-HTTPS absolute paths.
