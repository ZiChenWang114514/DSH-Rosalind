...
.preview-shell { box-sizing: border-box; min-height: 100vh; height: 1805px; padding: 48px 0 24px; background: radial-gradient(circle at 50% -10%, color-mix(in srgb, var(--rr-accent-soft) 58%, transparent), transparent 34%), var(--rr-bg); }
...
@media (max-width: 840px) { .preview-shell { height: 3108px; padding-top: 28px; } .preview-context { width: min(680px, calc(100vw - 28px)); } }
...
const scienceFixtures: Record<SciencePreview, { toolName: string; payload: Record<string, unknown> }> = {
  sequence: {
  ...
  ngs: {
    ...
    payload: {
      state: "running",
      registry_run_id: "run-rnaseq-042",
      workflow_id: "nf_core_rnaseq",
      engine: "nextflow",
      created_at: "2026-08-30T08:00:00Z",
      events: [
        { at: "2026-08-30T08:00:00Z", state: "queued", message: "Run accepted" },
        { at: "2026-08-30T08:00:22Z", state: "running", message: "Reference indices verified" },
        { at: "2026-08-30T08:01:04Z", state: "running", message: "Quantification processes active" },
      ],
      ...
      targets: [{ id: "local", title: "Local DSH host", kind: "local", ready: true }],
      artifacts: [{ path: "runs/run-rnaseq-042/execution.json", role: "execution record" }],
    },
  },
  structure: {
...
function SciencePreviewPage({ kind }: { kind: SciencePreview }): JSX.Element {
  ...
  const block = {
    ...
    callTime: 1,
    content: [{ type: "text" as const, text: JSON.stringify(fixture.payload) }],
    isError: false,
    meta: null,
    callView: null,
    resultView: null,
...
const scienceKind = science === "sequence" || science === "ngs" || science === "structure" || science === "slide" ? science : null;
createRoot(document.getElementById("root")!).render(scienceKind ? <SciencePreviewPage kind={scienceKind} /> : <Preview />);
