...
- Verifies the packaged plugin in the stock DSH settings interface and exercises the Science sidebar when the accompanying Harness patch is installed.
- Keeps bundled Skill documents byte-identical on Linux and Windows, and terminates complete POSIX workflow process groups during cancellation.
...
- Linux `npm run validate`: passed, including 52 Vitest files and 493 tests.
