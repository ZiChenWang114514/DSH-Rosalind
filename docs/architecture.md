...
The DSH sidebar shell and `ui-workspace` continue to own the brand row, New Session action, grouped or flat Workspace and Session rows, add, rename, reorder, search, fork, archive, Settings, theme, width, and collapsed rail. Rosalind does not replace `sidebar.workspaces` or the native Session Intent picker. The paired Harness change adds the typed `workspaceSidebar` service: Rosalind registers a `科学` view beside the permanent native `会话` view, while its existing `conversation.view` registration remains the central scientific workspace. Selecting scientific mode applies the registered Rosalind theme and both views; only a blank Session is recomposed, so a Session with history retains the capabilities that produced that history.
