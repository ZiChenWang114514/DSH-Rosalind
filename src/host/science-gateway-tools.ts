...
export function createScienceGatewayTools(executor: ScienceExecutor, packageRoot: string): ToolDefinition[] {
  return [
    defineTool({
      ...
      execute(args, exec) {
        return executeDeclared(executor, "literature", "literature.request", args, { session: exec.agent ?? FALLBACK_SESSION, signal: exec.signal, packageRoot, ...(args.allowNetwork !== undefined ? { allowNetwork: args.allowNetwork } : {}) });
      },
    ...
    defineTool({
      ...
      isConcurrencySafe: () => true,
      execute(args, exec) {
        return executeDeclared(executor, "databases", "database.request", args, { session: exec.agent ?? FALLBACK_SESSION, signal: exec.signal, packageRoot, ...(args.allowNetwork !== undefined ? { allowNetwork: args.allowNetwork } : {}) });
      },
