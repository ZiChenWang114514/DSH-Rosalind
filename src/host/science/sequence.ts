...
export class SequenceService {
  ...
  private open(args: Record<string, unknown>, context: ScienceExecutionContext): Json {
    ...
    const state: SequenceState = { viewerSessionId, sourcePath: path, sourceFormat: parsed.format, mode, records: parsed.records, options: { displayMode: "inline", toolbarVisible: true }, selectedRows: [], tracks: [], annotations: parsed.records.flatMap((record) => record.features.map((feature) => ({ record: record.id, ...feature }))), jobs: new Map(), analyses: [] };
  ...
  private control(args: Record<string, unknown>, context: ScienceExecutionContext): Json {
    ...
    } else if (action === "select_alignment_rows") state.selectedRows = Array.isArray(args.rows) ? args.rows.map((value) => String(value)) : [];
    else if (action === "select_sequence_range") state.selectedRange = { record: string(args.record, "record"), start: integer(args.start, "start"), end: integer(args.end, "end") };
    ...
    else if (action === "set_mode") state.mode = args.mode === "sequence" ? "sequence" : "alignment";
    else if (action === "set_display_mode" || action === "set_toolbar_visibility" || action.endsWith("_view_options") || action === "set_alignment_reference" || action === "set_workbench_panel" || action === "set_workbench_disclosure") Object.assign(state.options, object(args));
  ...
  private align(args: Record<string, unknown>, context: ScienceExecutionContext): Json {
    ...
    // Existing aligned records retain their scientific coordinates. For raw sequences we publish a deterministic left-padded exploratory alignment.
    const width = Math.max(...records.map((record) => record.sequence.length));
    const aligned = records.map((record) => ({ ...record, sequence: record.sequence.padEnd(width, "-") }));
    state.records = aligned; state.mode = "alignment";
    const job: SequenceJob = { id: randomUUID(), kind: String(args.algorithm ?? "builtin-center-star"), state: "completed", result: alignmentMetrics(aligned) };
    state.jobs.set(job.id, job);
    return { jobId: job.id, artifact: { format: "aligned-fasta", rowCount: aligned.length, alignedLength: width }, result: job.result };
  }
  ...
  private loadTrack(args: Record<string, unknown>, context: ScienceExecutionContext): Json {
    const state = this.state(args, context); const path = string(args.path, "path"); const format = string(args.format, "format");
    if (!existsSync(isAbsolute(path) ? path : resolve(context.packageRoot, path))) throw new Error(`Track source is unavailable: ${path}`);
    const track = { id: randomUUID(), path, format, reference: args.reference ?? null, start: args.start ?? null, end: args.end ?? null };
