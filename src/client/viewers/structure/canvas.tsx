...
export const STRUCTURE_CANVAS_RENDER_LIMIT = 12_000;

function sampledAtoms(atoms: readonly StructureCanvasAtom[]): readonly StructureCanvasAtom[] {
  if (atoms.length <= STRUCTURE_CANVAS_RENDER_LIMIT) return atoms;
  const sampled: StructureCanvasAtom[] = [];
  const step = atoms.length / STRUCTURE_CANVAS_RENDER_LIMIT;
  for (let index = 0; index < STRUCTURE_CANVAS_RENDER_LIMIT; index += 1) {
    const atom = atoms[Math.floor(index * step)];
    if (atom) sampled.push(atom);
  }
  return sampled;
}
...
export function LocalStructureCanvas(props: LocalStructureCanvasProps): JSX.Element {
  ...
  const renderAtoms = useMemo(() => sampledAtoms(props.atoms), [props.atoms]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const wheel = (event: WheelEvent) => {
      ...
      setViewport((current) => ({ ...current, scale: Math.max(.25, Math.min(8, current.scale * (event.deltaY > 0 ? .88 : 1.14))) }));
    };
  ...
  useEffect(() => {
    ...
    const dots = projected(renderAtoms, canvas, viewport);
  ...
  }, [canvasSize, props.background, renderAtoms, selected, viewport]);
  ...
  const nearest = (event: PointerEvent<HTMLCanvasElement>) => { const canvas = canvasRef.current; if (!canvas) return null; const at = point(event); return projected(renderAtoms, canvas, viewport).reduce<{ atom: StructureCanvasAtom; distance: number } | null>((best, dot) => { const distance = Math.hypot(dot.x - at.x, dot.y - at.y); return distance <= Math.max(10, dot.radius * 2) && (!best || distance < best.distance) ? { atom: dot.atom, distance } : best; }, null)?.atom ?? null; };
  const keyboard = (event: KeyboardEvent<HTMLCanvasElement>) => {
    const panStep = event.shiftKey ? 40 : 16;
    if (event.key === "Home" || event.key === "0") { event.preventDefault(); setViewport({ scale: 1, x: 0, y: 0 }); }
    else if (event.key === "Escape") props.onSelectAtom?.(null);
    else if (event.key === "+" || event.key === "=") { event.preventDefault(); setViewport((current) => ({ ...current, scale: Math.min(8, current.scale * 1.14) })); }
    else if (event.key === "-" || event.key === "_") { event.preventDefault(); setViewport((current) => ({ ...current, scale: Math.max(.25, current.scale * .88) })); }
    else if (event.key.startsWith("Arrow")) {
      event.preventDefault();
      setViewport((current) => ({ ...current, x: current.x + (event.key === "ArrowLeft" ? -panStep : event.key === "ArrowRight" ? panStep : 0), y: current.y + (event.key === "ArrowUp" ? -panStep : event.key === "ArrowDown" ? panStep : 0) }));
    }
  };
  ...
  return <canvas data-source-atom-count={props.atoms.length} data-rendered-atom-count={renderAtoms.length} data-view-scale={viewport.scale} ref={canvasRef} className="sv-structure-canvas" tabIndex={0} role="application" aria-label={props.ariaLabel ?? "Local molecular coordinate view. Drag or use arrow keys to pan, use the mouse wheel or plus and minus keys to zoom, click an atom to select it, and press Home to reset the view."} onKeyDown={keyboard} onPointerDown={(event) => { const canvas = canvasRef.current; if (!canvas) return; canvas.setPointerCapture?.(event.pointerId); const origin = point(event); dragRef.current = { pointerId: event.pointerId, origin, previous: origin, moved: false }; }} onPointerMove={(event) => { const drag = dragRef.current; if (!drag || drag.pointerId !== event.pointerId) return; const current = point(event); const deltaX = current.x - drag.previous.x, deltaY = current.y - drag.previous.y; if (Math.hypot(current.x - drag.origin.x, current.y - drag.origin.y) >= 4) drag.moved = true; setViewport((value) => ({ ...value, x: value.x + deltaX, y: value.y + deltaY })); drag.previous = current; }} onPointerUp={(event) => finishPointer(event, false)} onPointerCancel={(event) => finishPointer(event, true)} />;
}
