...
`skill-source-inventory.json` records the fixed plugin versions consulted for workflow compatibility. The distributed Skill documents are project-authored DSH-Rosalind instructions; the inventory stores separate hashes for the installed reference and the bundled document and does not imply that reference text is redistributed.
